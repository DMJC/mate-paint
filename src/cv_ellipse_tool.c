...
save_undo ( void )
{
    ...
    cairo_save (cr_mask);
    cairo_translate (cr_mask, x - rect.x + (w / 2.0), y - rect.y + (h / 2.0));
    cairo_scale (cr_mask, MAX (w / 2.0, 1.0), MAX (h / 2.0, 1.0));
    cairo_arc (cr_mask, 0, 0, 1, 0, 2 * G_PI);
    if ( m_priv->cv->filled != FILLED_NONE )
    {
        cairo_fill_preserve (cr_mask);
    }
